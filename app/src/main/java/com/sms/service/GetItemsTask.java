...
public class GetItemsTask extends AsyncTask<String, Void, Boolean> {
    ...
    private static final String TAG = GetItemsTask.class.getSimpleName();
    private Activity mContext;
    private String errorMessage;
    private MaterialDialog mPleaseWaitDialog;
    private AsyncTaskCompleteListener callback = null;

    public GetItemsTask(Activity mContext, AsyncTaskCompleteListener callback)  {
    ...
    protected Boolean doInBackground(String... params) {
        ...
        Log.d(TAG, "Starting getUpdatePresenter");
        NetworkResult<List<Presenter>> resultGetUpdate = App.getInstance().getNetworker().getUpdatePresenter(mContext);
        if (resultGetUpdate.isSuccess()){
            List<Presenter> result = resultGetUpdate.result;
            Log.d(TAG, "getUpdatePresenter executed succesfully.");
            try{
                ApiManager.processGetUpdatePresenter(result);
            } catch (ApiException e){
                Log.e(TAG, "Can't process getUpdatePresenter response", e);
            }
            return true;
        } else {
            if (resultGetUpdate.code == -101) {
                ...
                Log.d(TAG, "getUpdatePresenter failed. Status code: " + resultGetUpdate.code + ". Status message: " + resultGetUpdate.error);
